...
import Delcompexp from "./DeletecompExp"
import UpdateFormModalExp from "./UpdateFormModelExp"
import AnalysisExp from "./AnalysisExp"

export default function ExpenseListCard({ onBack }) {
  ...
  const fetchTotal = (params = {}) => {
    ...
    return axios.get(`https://trackmyspendapi-3.onrender.com/expense/totalexpense?${qs}`)
  }
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setExpenses(expenseRes.data.expenselist || [])
        setTotalExpense(Number(totalRes.data.totalExpense))
      } catch (err) {
        console.error(err)
      } finally {
        setLoading(false)
  ...
  useEffect(() => {
    if (query === "") {
      axios
        .get("https://trackmyspendapi-3.onrender.com/expense/viewexpense")
        .then((res) => {
          setExpenses(res.data.expenses || res.data.expenselist || [])
          return fetchTotal()
        })
        .then((res) => {
          setTotalExpense(Number(res.data.totalExpense))
        })
  ...
  const handleSearch = () => {
    ...
    axios
      .get(`https://trackmyspendapi-3.onrender.com/category/viewexpensecategory?category=${query}`)
      .then((res) => {
        setExpenses(res.data.categories || [])
        return fetchTotal(params)
      })
      .then((res2) => {
        setTotalExpense(Number(res2.data.totalExpense))
      })
  ...
  const applyFilter = () => {
    ...
    axios
      .get(`https://trackmyspendapi-3.onrender.com/category/expensefilter?${queryString}`)
      .then((res) => {
  ...
  return (
    <div className="flex-1 p-4 sm:p-6 w-full flex justify-center min-h-screen bg-gray-50 dark:bg-gray-900">
      <div className="w-full max-w-7xl">
        {viewanalysis ? (
          <AnalysisExp onBack1={() => setViewAnalysis(false)} />
        ) : (
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700">
            {/* Header */}
            <div className="p-4 sm:p-6 border-b border-gray-200 dark:border-gray-700">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <h2 className="text-2xl sm:text-3xl font-bold text-gray-800 dark:text-white">Expense Records</h2>
                <button
                  onClick={onBack}
                        ...
                        <tbody>
                          {expenses.map((inc, idx) => (
                            ...
                            >
                              ...
                              <td className="px-4 py-3 text-sm">{inc.to}</td>
                        ...
                        >
                          <div className="flex justify-between items-start mb-3">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="font-semibold text-gray-800 dark:text-white">{inc.to}</span>
                            </div>
                            <div className="flex items-center gap-2">
                              <button
                                onClick={() => {
                ...
                <div className="text-center">
                  <div className="inline-flex items-center gap-3 bg-green-50 dark:bg-green-900/20 px-6 py-4 rounded-lg">
                    ...
                    <div>
                      ...
                      <p className="text-2xl sm:text-3xl font-bold text-green-600">{formatCurrency(totalExpense)}</p>
                    </div>
                  </div>
                </div>
        ...
        {showupdate && recordToEdit && (
          <UpdateFormModalExp
            ...
            onSave={(updated) => {
              setExpenses((list) => list.map((i) => (i._id === updated._id ? updated : i)))
              setShowUpdate(false)
              fetchTotal()
                .then((res) => setTotalExpense(Number(res.data.totalExpense)))
